...
import ThingAddPopup from "@/views/thing-manage/ThingAddPopup.vue";
import ThingDetailPopup from "@/views/thing-manage/ThingDetailPopup.vue";
import {ref} from "vue";
...
const thingAddPopup = ref();
const thingDetailPopup = ref();

function onThingAdd(){
  thingAddPopup.value.open();
}

function onThingDetail(item){
  thingDetailPopup.value.open();
}
</script>

<template>
  <PageHeader title="Thing Management" :path-list="['자산 관리','시설 관리']">
    <template v-slot:button-area>
      <CBtnPrimary append-icon="mdi-plus" @click="onThingAdd" >Thing 등록</CBtnPrimary>
  ...
  <PageTableCard>
    ...
    <template v-slot:table-area>
      ...
      <tr v-for="i in 10">
        ...
        <td>789</td>
        <td>
          <CBtnIconViewCompact @click="onThingDetail"></CBtnIconViewCompact>
          <CBtnIconDeleteCompact></CBtnIconDeleteCompact>
          <CBtnIconEditCompact></CBtnIconEditCompact>
        </td>
      </tr>
      </tbody>
    </template>
  ...
  <ThingAddPopup ref="thingAddPopup"></ThingAddPopup>
  <ThingDetailPopup ref="thingDetailPopup"></ThingDetailPopup>
</template>
