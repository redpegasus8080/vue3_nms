...
import ServiceUserAddPopup from "./ServiceUserAddPopup.vue";
import {ref} from "vue";

const serviceUserAddPopup = ref();
function onUserAdd(){
  serviceUserAddPopup.value.open();
...
<template>
  <PageHeader title="Service User Management" :path-list="['사용자 관리','서비스 사용자']">
    <template v-slot:button-area>
      <CBtnPrimary append-icon="mdi-plus" @click="onUserAdd" >사용자 등록</CBtnPrimary>
  ...
  <PageTableCard>
    ...
    <template v-slot:table-area>
      ...
      <tr v-for="i in 10">
        ...
        <td>789</td>
        <td>
          <CBtnIconViewCompact @click="onUserDetail"></CBtnIconViewCompact>
  ...
  <ServiceUserAddPopup ref="serviceUserAddPopup"></ServiceUserAddPopup>
</template>
